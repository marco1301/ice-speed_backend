...
passings = ['ASCENDING'] #mettere solo ascending 'DESCENDING'
...
for area in areas[1:2]:
    ...
    with open("results_test"+area.get_glacier_name()+".txt","w") as file:
...
for area in areas[1:2]:
    ...
    print area.get_glacier_name()

    data = np.genfromtxt("results_test"+area.get_glacier_name()+".txt", skip_header=0)  
    x = data[:,0]
    y = data[:,1]
    ...
    graph_results = "./velocity/data/" + str(area.get_glacier_name() + "/")
    ...
    for ii in xrange((len(data[0,:])-2)/2):
        ...
        t2 = "{:%Y_%m_%d}".format(velocity_interval[ii+1][1])

        graph_results_date= join( graph_results, "graph" + t1 +"__"+ t2 + ".png")
        stampa(img_SAR,60,0,1, graph_results_date, x_im, y_im, vx, vy)
